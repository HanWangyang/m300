	...
	while (cfg->run_state != QUIT)
	{
		...
		if (dw > 0&&(strcmp((char *)inet_ntoa(addr.sin_addr), cfg->lidar_ip.c_str())==0))
		{
			...
			if (buf[0] == 0 && dw == packet_size)
			{
				...
				if(cfg->cloud_data.size()==0)
					cfg->frame_firstpoint_timestamp = packet->timestamp;

				else if(cfg->cloud_data.size() ==ONE_FRAME_POINT_NUM)
					cfg->frame_firstpoint_timestamp_cache = packet->timestamp;

				if(cfg->cloud_data.size()<ONE_FRAME_POINT_NUM)
					BlueSeaLidarSDK::getInstance()->AddPacketToList(packet, cfg->cloud_data,cfg->frame_firstpoint_timestamp);
				else if(cfg->cloud_data.size()>=ONE_FRAME_POINT_NUM)
				{
					cfg->frame_firstpoint_timestamp_cache = packet->timestamp;
					BlueSeaLidarSDK::getInstance()->AddPacketToList(packet, cfg->cloud_data_cache, cfg->frame_firstpoint_timestamp_cache);
				}
